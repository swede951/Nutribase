...
struct AddFoodView: View {
    ...
    @ObservedObject private var typesenseService = TypesenseService.shared
    private let novaScoreService = NovaScoreService.shared
    
    // Parameters for BasicFoodEntryView navigation
    ...
    // Navigation state for found food item
    @State private var foundFoodItem: FoodItem? = nil
    @State private var showingFoundFoodEntry = false
    ...
    var body: some View {
        NavigationView {
                ...
                Section(header: Text("Food Information")) {
                    ...
                    HStack {
                        ...
                        if !barcode.isEmpty {
                            ...
                            .help("Search for food by barcode")
                ...
                Section(header: Text("Sharing Options")) {
                    HStack(alignment: .center, spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Added to global food database")
                                .font(.body)
                            
                            Text("This food will be available to all Nutribase users")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                        .padding(.vertical, 4)
            ...
            .navigationDestination(isPresented: $showingFoundFoodEntry) {
                if let food = foundFoodItem {
                    BasicFoodEntryView(
                        ...
                        onFoodAdded: { _ in
                            // Dismiss the AddFoodView after adding the food
                            presentationMode.wrappedValue.dismiss()
                        },
    ...
    private func lookupFoodByBarcode(_ barcode: String) {
        ...
        // Barcode search disabled
        isSubmitting = false
        alertMessage = "Barcode search is currently disabled"
        showingAlert = true
    }
    ...
    private func submitFood() {
        ...
        // Food submission disabled
        print("📡 Food submission disabled")
        DispatchQueue.main.async {
            self.isSubmitting = false
            self.alertMessage = "Food submission is currently disabled"
    ...
    private func handleError(_ message: String) {
        print("❌ Failed to add food item: \(message)")
...
// Note: Barcode scanner components moved to BarcodeScannerComponents.swift for global access
