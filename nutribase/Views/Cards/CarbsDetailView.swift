...
struct CarbsDetailView: View {
    ...
    @State private var monthlyChartDataCache: [Int: [MonthDayCarbsData]] = [:]
    ...
    private func computeWeeklyData(for offset: Int) async -> WeeklyMacroData {
        ...
        for day in days {
            ...
            let carbs = foodLogManager.totalCarbsForDay(date: date)
    ...
    private func computeMonthlyData(for offset: Int) async -> MonthlyMacroData {
        ...
        for day in 1...monthRange.count {
            ...
            let carbs = foodLogManager.totalCarbsForDay(date: dayDate)
    ...
    struct DayCarbsData: Identifiable {
        let id = UUID()
        let day: String
        let dayIndex: Int
        let grams: Int
    ...
    private func getWeeklyChartData(for offset: Int) -> [DayCarbsData] {
        let days = ["M", "Tu", "W", "Th", "F", "Sa", "Su"]
    ...
    struct MonthDayCarbsData: Identifiable {
    ...
    private func getMonthlyChartData(for offset: Int) -> [MonthDayCarbsData] {
        ...
        return getDaysOfMonth(for: offset).compactMap { day in
            ...
            let carbs = cachedData?.dailyData[day] ?? 0
            let hasEntries = cachedData?.hasEntries[day] ?? false
            
            return MonthDayCarbsData(
                date: date,
                day: day,
                grams: carbs,
                hasEntries: hasEntries
    ...
    private func buildMonthlyChartDataCache(for offset: Int) {
        ...
        let cachedData = monthlyDataCache[offset]
        let result = getDaysOfMonth(for: offset).compactMap { day -> MonthDayCarbsData? in
            guard let date = calendar.date(byAdding: .day, value: day - 1, to: targetMonthStart) else {
                return nil
            }
            let carbs = cachedData?.dailyData[day] ?? 0
    ...
    private func weeklyBarChart(for offset: Int) -> some View {
        ...
        return VStack(spacing: 20) {
            GeometryReader { geometry in
                ...
                .coordinateSpace(name: "carbsChartSpace")
            }
            .frame(height: 180)
    ...
    private func monthlyBarChart(for offset: Int) -> some View {
        ...
        let maxValue = carbsTarget * 12 / 10
        let roundedMax = Int(ceil(Double(maxValue) / 10.0) * 10)
        let barColor = CardType.carbs.color
    ...
    private func getMonthlyDailyValue(day: Int, monthOffset: Int) -> Int {
        ...
        return Int(dayEntries.reduce(0.0) { $0 + $1.totalCarbs })
    }
    ...
    private func loadFromSharedCache(weekOffset: Int) -> WeeklyMacroData? {
        guard let cached = cacheManager.getWeeklyMacroCache(for: .carbs, weekOffset: weekOffset) else {
            return nil
        }
        return WeeklyMacroData(
    ...
    private func saveToSharedCache(_ data: WeeklyMacroData, weekOffset: Int) {
        ...
        cacheManager.setWeeklyMacroCache(cache, for: .carbs)
    }
    
    /// Load monthly data from shared cache
    private func loadMonthlyFromSharedCache(monthOffset: Int) -> MonthlyMacroData? {
        guard let cached = cacheManager.getMonthlyMacroCache(for: .carbs, monthOffset: monthOffset) else {
            return nil
        }
    ...
    private func saveMonthlyToSharedCache(_ data: MonthlyMacroData, monthOffset: Int) {
        ...
        cacheManager.setMonthlyMacroCache(cache, for: .carbs)
    }
}
