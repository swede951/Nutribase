...
struct ProteinDetailView: View {
    ...
    @State private var monthlyChartDataCache: [Int: [MonthDayProteinData]] = [:]
    ...
    var body: some View {
        ...
        NavigationView {
            ...
            .onChange(of: viewMode) { oldMode, newMode in
                if newMode == .monthly {
                    // Preload monthly data when switching to monthly view
                    preloadMonthlyData()
                }
            }
        }
            ...
            Task(priority: .userInitiated) {
                if viewMode == .weekly {
                    ...
                    if monthlyDataCache[offset] == nil {
                        ...
                        // Data cached but chart data not built yet
                        buildMonthlyChartDataCache(for: offset)
                    }
                }
            }
    ...
    private func computeWeeklyData(for offset: Int) async -> WeeklyMacroData {
        ...
        for day in days {
            ...
            let protein = foodLogManager.totalProteinForDay(date: date)
    ...
    private func computeMonthlyData(for offset: Int) async -> MonthlyMacroData {
        ...
        for day in 1...monthRange.count {
            ...
            let protein = foodLogManager.totalProteinForDay(date: dayDate)
    ...
    struct DayProteinData: Identifiable {
        let id = UUID()
        let day: String
        let dayIndex: Int
        let grams: Int
    ...
    private func getWeeklyChartData(for offset: Int) -> [DayProteinData] {
        let days = ["M", "Tu", "W", "Th", "F", "Sa", "Su"]
    ...
    struct MonthDayProteinData: Identifiable {
    ...
    private func getMonthlyChartData(for offset: Int) -> [MonthDayProteinData] {
        // Return cached chart data if available
        if let cached = monthlyChartDataCache[offset] {
            return cached
        }
        
        // Build chart data from monthly data cache
        let calendar = Calendar.current
        let today = Date()
    ...
    private func weeklyBarChart(for offset: Int) -> some View {
        ...
        return VStack(spacing: 20) {
            GeometryReader { geometry in
                ...
                .coordinateSpace(name: "proteinChartSpace")
            }
            .frame(height: 180)
    ...
    private func monthlyBarChart(for offset: Int) -> some View {
        ...
        let maxValue = proteinTarget * 12 / 10
        let roundedMax = Int(ceil(Double(maxValue) / 10.0) * 10)
        let barColor = CardType.protein.color
    ...
    private func getMonthlyDailyValue(day: Int, monthOffset: Int) -> Int {
        ...
        return Int(dayEntries.reduce(0.0) { $0 + $1.totalProtein })
    }
    ...
    private func loadFromSharedCache(weekOffset: Int) -> WeeklyMacroData? {
        guard let cached = cacheManager.getWeeklyMacroCache(for: .protein, weekOffset: weekOffset) else {
            return nil
        }
        return WeeklyMacroData(
    ...
    private func saveToSharedCache(_ data: WeeklyMacroData, weekOffset: Int) {
        ...
        cacheManager.setWeeklyMacroCache(cache, for: .protein)
    }
    
    /// Load monthly data from shared cache
    private func loadMonthlyFromSharedCache(monthOffset: Int) -> MonthlyMacroData? {
        guard let cached = cacheManager.getMonthlyMacroCache(for: .protein, monthOffset: monthOffset) else {
            return nil
        }
    ...
    private func saveMonthlyToSharedCache(_ data: MonthlyMacroData, monthOffset: Int) {
        ...
        cacheManager.setMonthlyMacroCache(cache, for: .protein)
    }
}

// Tooltip view for displaying daily macro values on long press
struct MacroTooltip: View {
    @Environment(\.colorScheme) private var colorScheme
    ...
    var percentage: Int {
        ...
        return Int((Double(value) / Double(target)) * 100)
    }
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 8) {
                ...
                Text("\(percentage)%")
                    .font(.caption)
                    .fontWeight(.bold)
                    .foregroundColor(percentage >= 100 ? .green : color)
            }
        }
        .padding(12)
        .background(
