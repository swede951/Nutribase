        ...
        NavigationView {
            ...
            .navigationDestination(isPresented: $showingFoodEntry) {
                if let food = selectedFood {
                    BasicFoodEntryView(food: food, mealType: mealType, selectedDate: selectedDate, onFoodAdded: addToRecentFoods)
                } else {
...
struct FoodItemCard: View {
    ...
    var body: some View {
        ...
        .padding(.horizontal, 8) // Increased padding for shadow visibility
    ...
    private func displayServingSize() -> String {
        if let cachedSize = food.cachedServingSize,
           ...
           let cachedServings = food.cachedNumberOfServings {
            // Show total consumed amount (serving size × number of servings)
            ...
            return "\(formattedSize)\(cachedUnit)"
        } else {
    ...
    // Display calories - either calculated from cached serving or default
    private func displayCalories() -> Int {
        if let cachedServings = food.cachedNumberOfServings {
            // Calculate calories based on cached number of servings
            return Int(round(Double(food.calories) * cachedServings))
        } else {
            // Use default calculation
            return calculateDefaultServingCalories(for: food)
        }
    }
}
...
public struct FoodItem: Identifiable, Codable, Equatable {
    ...
    enum CodingKeys: String, CodingKey {
        case id, name, brandName, barcode, calories, protein, carbs, fat, novaScore, nutriScoreGrade, servingSize, servingsPerPackage, servingType, cachedServingSize, cachedServingUnit, cachedNumberOfServings
    }
